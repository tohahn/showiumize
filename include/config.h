...
conf_config* read_config_file() {
	...
	// reading config file
	conf_config* config = malloc(sizeof(conf_config));
	char*** key_values = extract_lines(conf_file);

	write_log("Reading configuration values.");
...
char*** extract_lines(FILE* conf_file) {
	...
	for (int i = 0; i < CONF_NUMBER_VALUES; i++) {
		...
		*newline = '\0';
		...
		if ((line = strsep(&line, CONF_KEY_VALUE_SEPARATOR))) {
...
unsigned char change_config_dir() {
	...
	if (conf_xdg_conf) {
		temp_dir = (char *) malloc((strlen(conf_xdg_conf) + sizeof(CONF_CONF_FOLDER) + 2) * sizeof(char));
		sprintf(temp_dir, "%s/%s", conf_xdg_conf, CONF_CONF_FOLDER);
	} else {
		write_error("No XDG_CONFIG_HOME variable, using $HOME/.config instead.");
		...
		temp_dir = (char *) malloc((strlen(conf_home) + sizeof(CONF_CONF_DIR) + sizeof(CONF_CONF_FOLDER) + 3) * sizeof(char));
